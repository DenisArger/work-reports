...
export default async function handler(req) {
    ...
    try {
        if (update.message) {
            ...
            switch (command) {
                case '/start': {
                    await tgSendMessage(chatId, `🎉 *Привет, ${userName}!*\\n\\n` +
                ...
                case '/help': {
                    await tgSendMessage(chatId, `📚 *Справка по командам*\\n\\n` +
                        `/start - Начать работу\\n` +
                ...
                case '/reports': {
                    ...
                    const msg = `📊 *Найдено отчетов: ${reports.length}*\\n\\n` +
                        top
        ...
        try {
            ...
            if (adminChatId) {
                await tgSendMessage(adminChatId, `❌ *Ошибка*\\n` +
